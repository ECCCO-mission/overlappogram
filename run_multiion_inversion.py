...
def run_inversion(image_path, config: dict):
    ...
    inversion.initialize_input_data(image_path,
                                    None,
    ...
    for alpha in config['model']['alphas']:
        for rho in config['model']['rhos']:
            ...
            postfix = 'x'+str(config['inversion']['solution_fov_width'])+'_'+str(rho*10)+'_'+str(alpha)+'_wpsf'
            em_cube_paths.append(inversion.multiprocessing_invert(config['model'],
                                             alpha,
                                             rho,
...
if __name__ == "__main__":
    ...
    if config['output']['make_spectral']:
        ...
        mdp.create_level2_0_spectrally_pure_images(em_cube_paths,
                                                   config['paths']['gnt'],

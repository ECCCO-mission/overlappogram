...
class Inversion:
    ...
    def __post_init__(self):
        ...
        response_count = 0
        self.response_function = np.zeros((self.num_selections  * self.num_slits, self.rsp_func_width), dtype=np.float32)
    ...
    def invert(self, model, output_dir: str,
        ...
        em_data_cube = np.zeros((self.image_height, self.num_slits, len(self.inv_logt_list), len(self.inv_ion_list)), dtype=np.float32)
        em_data_cube[:, :, :, :] = -1.0
            ...
            with warnings.catch_warnings():
                ...
                try:
                    ...
                    if self.smooth_over == 'dependence':
                        for slit_num in range(self.num_slits):
                            for index in range(self.num_selections):
                                em_data_cube[image_row_number, slit_num, self.inverted_selection[0][index], self.inverted_selection[1][index]] = em[response_count]
                                if image_row_number == 512:
                                    print("*", image_row_number, slit_num, self.inverted_selection[0][index], self.inverted_selection[1][index], response_count)
                                response_count += 1
                    else:
                        for index in range(self.num_selections):
                            for slit_num in range(self.num_slits):
    ...
    #     #         pool.apply_async(self.multiprocessing_invert_image_row, args = (i, model), callback = self.multiprocessing_callback)
    ...
    def create_forward_model(self, em_data_cube_file: str, output_dir: str, image_mask_file: str = None):
        ...
        if image_mask is None:
            ...
            if self.smooth_over == 'dependence':
                ...
                for slit_num in range(self.num_slits):
                    for index in range(self.num_selections):
                        forward_model_image += np.dot(em_data_cube[self.inverted_selection[1][index], self.inverted_selection[0][index], :, slit_num][:, None], self.response_function[:, response_count][None, :])
                        response_count += 1
            else:
                self.smooth_over = 'spatial'
                # Smooth over spatial.
            ...
            for image_row_number in range(499,551):#em_rows):
                ...
                if self.smooth_over == 'dependence':
                    ...
                    for slit_num in range(self.num_slits):
                        for index in range(self.num_selections):
                            forward_model_image[image_row_number, :] += np.dot(em_data_cube[self.inverted_selection[1][index], self.inverted_selection[0][index], image_row_number, slit_num], masked_rsp_func[:, response_count])
                            response_count += 1
                else:
                    self.smooth_over = 'spatial'
                    # Smooth over spatial.

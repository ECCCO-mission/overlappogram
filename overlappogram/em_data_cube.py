...
def create_background(
    num_x: int, num_y: int, background_value: float, add_noise: bool = False
) -> np.ndarray:
    """
...
class EMDataCube:
    ...
    def add_explosive_event(self, filename: str, locations: list):
        ...
        for x, y in locations:
            ...
            if (pixel_x >= 0 and pixel_x < num_x) and (
                pixel_y >= 0 and pixel_y < num_y
            ):
    ...
    def create_simulated_data(self, image_list: list):
        ...
        for i in range(len(x_pixels)):
            ...
            world_y, world_x, world_vel = self.cube.pixel_to_world(
                y * u.pix, x * u.pix, z * u.pix
            )
            for image in image_list:
                image.add_simulated_data(
                    world_x, world_y, world_vel, self.cube.data[y, x, z]
                )
    ...
    def prep_inversion(self, image_list):
        ...
        for image in image_list:
            self.inversion_image_list.append(
                image.crop_roi([min_world_y, min_world_x], [max_world_y, max_world_x])
            )
        ...
        for j in range(0, num_y):
            for i in range(0, num_x):
                for k in range(0, num_vel):
                    y_out, x_out, vel_out = self.cube.pixel_to_world(
    ...
    def invert_data1(
        ...
        slope=0.0,
        bias=1.0,
        inversion_image_data_list: list[np.ndarray] | None= None,
    ):
        num_y, num_x, num_vel = self.cube.wcs.array_shape
        ...
        X, Y = np.meshgrid(
            np.linspace(0, x, len(intensity_values[0, :])),
            np.linspace(0, y, len(intensity_values[:, 0])),
        )
        fig = plt.figure()
        ax = fig.add_subplot(111, projection="3d")
        ax.scatter3D(X, Y, intensity_values)
        ...
        for x in range(num_x):
            for y in range(num_y):
                ...
                if intensity != 0.0:
                    ...
                    skew = np.sum(
                        self.cube.data[y, x, :] * (velocities - vbar) ** 3.0 / intensity
                    )
                    if skew < 0.0:
                        skew = (abs(skew) ** (1.0 / 3.0)) * -1.0
                    else:
                        skew = skew ** (1.0 / 3.0)
                    intensity_values[y, x] = intensity

...
class MaGIXSDataProducts():
    ...
    def replace_bad_pixels(self, image: np.ndarray, bad_pixel_mask: np.ndarray) -> [np.ndarray, pd.DataFrame]:
        '''
        ...
        if len(bad_pixel_list[0]) > 0:
            ...
            data = np.zeros((len(bad_pixel_list[0]),), dtype=[("y", np.int32), ("x", np.int32), ("value", np.float32)])
            ...
            for c in range(len(bad_pixel_list[0])):
                ...
                for i in [-1, 0, 1]:
                    for j in [-1, 0, 1]:
                        ...
                        if 0 <= bad_pixel_list[0][c] + i < height and 0 <= bad_pixel_list[1][c] + j < width:
                            adjacent_pixel_list.append(image[bad_pixel_list[0][c] + i, bad_pixel_list[1][c] + j])
    ...
    def create_bad_pixel_mask(self, image_list: list, pre_master_dark_file: str, post_master_dark_file: str, sigma: np.float32, percent_threshold: np.float32, bad_pixel_mask_file: str):
        ...
        bad_pixels = np.zeros((number_images, self.active_pixels_height, self.active_pixels_width), dtype=np.float32)
        ...
        bad_pixel_mask_header['IMGSEQNM'] = (image_sequence_numbers, 'Image Sequence Numbers')
        bad_pixel_mask_header ['SIGMA'] = (sigma, 'Sigma')
        bad_pixel_mask_header['PCNTTHLD'] = (percent_threshold, 'Percent Threshold')
        bad_pixel_mask_header['PREMDARK'] = (os.path.basename(pre_master_dark_file), 'Pre Master Dark')
        bad_pixel_mask_header['PSTMDARK'] = (os.path.basename(post_master_dark_file), 'Post Master Dark')
    ...
    def create_bad_pixel_mask_by_tap(self, image_list: list, sigma: np.float32, percent_threshold: np.float32, bad_pixel_mask_file: str):
        ...
        bad_pixels[np.where(np.isnan(image_cube))] = np.nan
        for index in range(number_images):
            bad_pixels[index, 0:quad_height, 0:quad_width][np.where(image_cube[index, 0:quad_height, 0:quad_width] < np.nanmedian(image_cube[index, 0:quad_height, 0:quad_width]) - (sigma * np.nanstd(image_cube[index, 0:quad_height, 0:quad_width])))] = 1
            bad_pixels[index, 0:quad_height, quad_width:self.active_pixels_width][np.where(image_cube[index, 0:quad_height, quad_width:self.active_pixels_width] < np.nanmedian(image_cube[index, 0:quad_height, quad_width:self.active_pixels_width]) - (sigma * np.nanstd(image_cube[index, 0:quad_height, quad_width:self.active_pixels_width])))] = 1
            bad_pixels[index, quad_height:self.active_pixels_height, 0:quad_width][np.where(image_cube[index, quad_height:self.active_pixels_height, 0:quad_width] < np.nanmedian(image_cube[index, quad_height:self.active_pixels_height, 0:quad_width]) - (sigma * np.nanstd(image_cube[index, quad_height:self.active_pixels_height, 0:quad_width])))] = 1
            bad_pixels[index, quad_height:self.active_pixels_height, quad_width:self.active_pixels_width][np.where(image_cube[index, quad_height:self.active_pixels_height, quad_width:self.active_pixels_width] < np.nanmedian(image_cube[index, quad_height:self.active_pixels_height, quad_width:self.active_pixels_width]) - (sigma * np.nanstd(image_cube[index, quad_height:self.active_pixels_height, quad_width:self.active_pixels_width])))] = 1
            bad_pixels[index, 0:quad_height, 0:quad_width][np.where(image_cube[index, 0:quad_height, 0:quad_width] > np.nanmedian(image_cube[index, 0:quad_height, 0:quad_width]) + (sigma * np.nanstd(image_cube[index, 0:quad_height, 0:quad_width])))] = 1
            bad_pixels[index, 0:quad_height, quad_width:self.active_pixels_width][np.where(image_cube[index, 0:quad_height, quad_width:self.active_pixels_width] > np.nanmedian(image_cube[index, 0:quad_height, quad_width:self.active_pixels_width]) + (sigma * np.nanstd(image_cube[index, 0:quad_height, quad_width:self.active_pixels_width])))] = 1
            bad_pixels[index, quad_height:self.active_pixels_height, 0:quad_width][np.where(image_cube[index, quad_height:self.active_pixels_height, 0:quad_width] > np.nanmedian(image_cube[index, quad_height:self.active_pixels_height, 0:quad_width]) + (sigma * np.nanstd(image_cube[index, quad_height:self.active_pixels_height, 0:quad_width])))] = 1
            bad_pixels[index, quad_height:self.active_pixels_height, quad_width:self.active_pixels_width][np.where(image_cube[index, quad_height:self.active_pixels_height, quad_width:self.active_pixels_width] > np.nanmedian(image_cube[index, quad_height:self.active_pixels_height, quad_width:self.active_pixels_width]) + (sigma * np.nanstd(image_cube[index, quad_height:self.active_pixels_height, quad_width:self.active_pixels_width])))] = 1
    ...
    def create_master_dark(self, image_list: list, sigma: np.float32, master_dark_file: str):
        '''
        ...
        number_images = len(image_list)
        image_cube = np.zeros((number_images, self.active_pixels_height, self.active_pixels_width), dtype=np.float32)
        master_dark = np.zeros((self.active_pixels_height, self.active_pixels_width), dtype=np.float32)
        ...
        # Despike.
        image_cube[np.where(image_cube > (np.nanmedian(image_cube, axis=0) + (sigma * np.nanstd(image_cube, axis=0))))] = np.nan
        ...
        master_dark_header['IMG_EXP'] = (summed_image_exposure_time / number_images, 'Exposure (seconds)')
        master_dark_header['MEAS_EXP'] = (summed_measured_exposure_time / number_images, 'Measured Exposure (seconds)')
        master_dark_header['FPGATEMP'] = (summed_fpga_temp / number_images, 'FPGA Temperature (degC)')
        master_dark_header['FPGAVINT'] = (summed_fpga_vint / number_images, 'FPGA VccInt Voltage (volts)')
        ...
        image_sequence_numbers = self.create_image_sequence_number_list(img_seq_num_list)
        master_dark_header['IMGSEQNM'] = (image_sequence_numbers, 'Image Sequence Numbers')
    ...
    def create_level0_5_images(self, image_list: list, output_dir: str):
        ...
        t0_delta = (launch_t0 - timedelta(days=1).total_seconds()) - exp_launch_time.total_seconds()
        ...
        for index in range(number_images):
            with fits.open(image_list[index]) as image_hdul:
                ...
                date_obs_image_date = image_date + timedelta(seconds=t0_delta) - timedelta(seconds=measured_exposure)
                image_hdul[0].header['DATE_OBS'] = (date_obs_image_date.isoformat(timespec='milliseconds').replace("+00:00", "Z"), 'Date Observation')
                t_obs_image_date = date_obs_image_date + timedelta(seconds=(measured_exposure / 2.0))
                image_hdul[0].header['T_OBS'] = (t_obs_image_date.isoformat(timespec='milliseconds').replace("+00:00", "Z"), 'Telescope Observation')
                ...
                image_hdul[0].header['COLDBLOC'] =( 0.0, 'Cold Block Temperature (degC)')
                image_hdul[0].header['UNITS'] = ('DN', 'Units')

                image_timestamp = date_obs_image_date.isoformat(timespec='milliseconds').replace("+00:00", "Z")
                # Create output directory.
                os.makedirs(output_dir, exist_ok=True)
                output_file = output_dir + 'magixs_L0.5_' + image_timestamp.replace(":", ".") + '.fits'
                hdu = fits.PrimaryHDU(data = image_hdul[0].data, header = image_hdul[0].header)
                hdulist = fits.HDUList([hdu])
                hdulist.writeto(output_file, overwrite=True)

    def update_level0_5_image_acquisition_state(self, input_dir: str, image_sequence_number_list: list, image_acquisition_state: str):
        '''
    ...
    def update_level0_5_temperatures(self, input_dir: str, matlab_file: str):
        ...
        for file in glob.glob("*.fits"):
            with fits.open(file) as image_hdul:
                ...
                hdulist = fits.HDUList([hdu])
                hdulist.writeto(file, overwrite=True)
                #print(image_delta, ccd_param_cal[index], ccd_param_time[index], cold_block_param_cal[index], cold_block_param_time[index])

    def create_level1_0_images(self, image_list: list, pre_master_dark_file: str, post_master_dark_file: str, bad_pixel_mask_file: tp.Union[str, None], output_dir: str):
        '''
        ...
        despike_table_hdu = fits.BinTableHDU.from_columns([fits.Column(name='y', format='J'),
                                                           fits.Column(name='x', format='J'),
                                                           fits.Column(name='value', format='E')])
        wcs_table_hdu = fits.BinTableHDU.from_columns([fits.Column(name='wavelength' , format='E'),
        ...
        for index in range(number_images):
            with fits.open(image_list[index]) as image_hdul:
                image, image_header, bad_pixels_replaced_values = self.create_adjusted_light(image_hdul[0].data, image_hdul[0].header, pre_master_dark, pre_dark_temp, post_master_dark, post_dark_temp, bad_pixel_mask)
                image_header['PREMDARK'] = (os.path.basename(pre_master_dark_file), 'Pre Master Dark')
                image_header['PSTMDARK'] = (os.path.basename(post_master_dark_file), 'Post Master Dark')
                image_header['INTPTEMP'] = ('ADCTEMP6', 'Interpolation Temperature')

                # Create output directory.
                os.makedirs(output_dir, exist_ok=True)
                image_date = dateutil.parser.isoparse(image_hdul[0].header['DATE_OBS'])
                image_timestamp = image_date.isoformat(timespec='milliseconds').replace("+00:00", "Z")
                output_file = output_dir + 'magixs_L1.0_' + image_timestamp.replace(":", ".") + '.fits'
                fits_hdu = fits.PrimaryHDU(data = image, header = image_header)
                bad_pixels_replaced_dict = {'y':np.int32, 'x':np.int32, 'value':np.float32}
                bad_pixels_replaced_values = bad_pixels_replaced_values.astype(bad_pixels_replaced_dict)
                bad_pixels_table = Table.from_pandas(bad_pixels_replaced_values)
                hdu_list = fits.HDUList([fits_hdu, fits.table_to_hdu(bad_pixels_table), despike_table_hdu, wcs_table_hdu])
                hdu_list.writeto(output_file, overwrite=True)

    def create_adjusted_light(self, image: np.ndarray, level0_header: astropy.io.fits.header.Header, pre_master_dark: np.ndarray, pre_master_dark_temp: np.float32, post_master_dark: np.ndarray, post_master_dark_temp: np.float32, bad_pixel_mask: tp.Union[np.ndarray, None]=None) -> [np.ndarray, astropy.io.fits.header.Header, pd.DataFrame]:
        '''
        ...
        adjusted_image = np.zeros((self.active_pixels_height, self.active_pixels_width), dtype=np.float32)
        ...
        #print("pre", adjusted_image[544, 1517])
        # if image_temp < pre_master_dark_temp:
        #     master_dark_scale = 0.0
        # elif image_temp > post_master_dark_temp:
        #     master_dark_scale = 1.0
        # else:
        #     master_dark_scale = 1.0 - ((post_master_dark_temp - image_temp)/(post_master_dark_temp - pre_master_dark_temp))
        #master_dark_scale = 1.0 - ((post_master_dark_temp - image_temp)/(post_master_dark_temp - pre_master_dark_temp))
        master_dark_scale = (image_temp - pre_master_dark_temp )/(post_master_dark_temp - pre_master_dark_temp)
        #print(master_dark_scale)
        scaled_master_dark = pre_master_dark.astype(np.float32) + (master_dark_scale * (post_master_dark.astype(np.float32) - pre_master_dark.astype(np.float32)))
        ...
        if bad_pixel_mask is not None:
            adjust_image, bad_pixels_replaced_values = self.replace_bad_pixels(adjusted_image, bad_pixel_mask)
    ...
    def create_level1_5_images(self, image_list: list, despike_areas: list, despike_sigma: np.float32, output_dir: str):
        '''
        ...
        for index in range(number_images):
            with fits.open(image_list[index]) as image_hdul:
                image, image_header, spike_replaced_values = self.create_inversion_ready_light_with_mask(image_hdul[0].data, image_hdul[0].header, despike_areas, despike_sigma)
                # Create output directory.
                os.makedirs(output_dir, exist_ok=True)
                image_date = dateutil.parser.isoparse(image_hdul[0].header['DATE_OBS'])
                image_timestamp = image_date.isoformat(timespec='milliseconds').replace("+00:00", "Z")
                output_file = output_dir + 'magixs_L1.5_' + image_timestamp.replace(":", ".") + '.fits'
    ...
    def despike_pixel_with_mask(self, despiked_image: np.ndarray, y_pixel: np.float32, x_pixel: np.float32, despike_threshold: np.float32, despike_sigma: np.float32, spikes: list) -> [np.ndarray, pd.DataFrame]:
        '''
        ...
        for index, delta in enumerate(pixel_16_adjacent_deltas):
            ...
            if skip:
                ...
                if (y,x) not in spikes:
                    ...
                    skip = False
                continue
            if (y >= 0 and y < self.active_pixels_height) and (x >= 0 and x < self.active_pixels_width):
        ...
        if len(surrounding_pixels_list) > 1:
            ...
            if despiked_image[y_pixel, x_pixel] > np.median(surrounding_pixels_values) + (despike_sigma * np.std(surrounding_pixels_values)):
                #despiked_image[y_pixel, x_pixel] = np.mean(surrounding_pixels_values)
                spike_replaced_values.loc[len(spike_replaced_values.index)] = [y_pixel, x_pixel, despiked_image[y_pixel, x_pixel]]
                ...
                #for delta in pixel_16_deltas:
                    y = y_pixel + delta[0]
                    x = x_pixel + delta[1]
                    if (y >= 0 and y < self.active_pixels_height) and (x >= 0 and x < self.active_pixels_width) and (despiked_image[y, x] < despike_threshold) and despiked_image[(y, x)] is not ma.masked:
                        despiked_image, area_spike_values_replaced = self.despike_pixel_with_mask(despiked_image, y, x, despike_threshold, despike_sigma, spikes)
                        spike_replaced_values = spike_replaced_values.append(area_spike_values_replaced, ignore_index=True)
        elif len(surrounding_pixels_list) == 1:
            if despiked_image[y_pixel, x_pixel] > despiked_image[surrounding_pixels_list[0]]:
                spike_replaced_values.loc[len(spike_replaced_values.index)] = [y_pixel, x_pixel, despiked_image[y_pixel, x_pixel]]
                #print(y_pixel, x_pixel, despiked_image[y_pixel, x_pixel])
                despiked_image[y_pixel, x_pixel] = despiked_image[surrounding_pixels_list[0]]
        else:
            surrounding_pixels_values = []
            for index, delta in enumerate(pixel_16_adjacent_deltas):
                y = y_pixel + delta[0]
    ...
    def despike_with_mask(self, image: ma.masked_array, despike_threshold: np.float32, despike_sigma: np.float32) -> [np.ndarray, pd.DataFrame]:
        '''
        ...
        while spike_values:
            ...
            despiked_image, area_spike_replaced_values = self.despike_pixel_with_mask(despiked_image, y_pixel, x_pixel, despike_threshold, despike_sigma, original_spikes)
            spike_replaced_values = spike_replaced_values.append(area_spike_replaced_values, ignore_index=True)
            del spikes[min_spike_index]
            del spike_values[min_spike_index]

        return despiked_image.data, spike_replaced_values

    def create_inversion_ready_light_with_mask(self, image: np.ndarray, level1_header: astropy.io.fits.header.Header, despike_areas: list, despike_sigma: np.float32) -> [np.ndarray, astropy.io.fits.header.Header, pd.DataFrame]:
        '''
        ...
        '''
        image_height, image_width = np.shape(image)
        inversion_ready_image = np.zeros((self.active_pixels_height, self.active_pixels_width), dtype=np.float32)
        spike_replaced_values = pd.DataFrame(columns=['y', 'x', 'value'])
        mask = np.zeros((self.active_pixels_height, self.active_pixels_width), dtype=np.bool)
        ...
        for despike_area in despike_areas:
            ...
            for area in despike_area[0]:
                y0 = area[0][0]
                y1 = area[0][1]
                assert(y1 > y0 and y0 >= 0 and y1 <= self.active_pixels_height)
                x0 = area[1][0]
                x1 = area[1][1]
                assert(x1 > x0 and x0 >= 0 and x1 <= self.active_pixels_width)
                #print(y0, y1, x0, x1, despike_threshold)
                mask[y0:y1, x0:x1] = False

            masked_image = ma.masked_array(inversion_ready_image, mask=mask, fill_value = np.nan)
            inversion_ready_image, area_spike_replaced_values = self.despike_with_mask(masked_image, despike_threshold, despike_sigma)

            spike_replaced_values = spike_replaced_values.append(area_spike_replaced_values, ignore_index=True)

        return inversion_ready_image, level1_5_header,  spike_replaced_values

    def update_level1_pointing(self, image_list: list, level1_wcs_table: str, solar_fov_coords: str):
        number_images = len(image_list)

        # Read Level 1 WCS table information.
        wcs_table = pd.read_excel(level1_wcs_table, index_col=None)
        rows, columns = wcs_table.shape
        assert rows >= 1 and columns == 8 and wcs_table.columns[0] == 'wavelength'
        # Read solar FOV coordinates.
        fov_table = pd.read_excel(solar_fov_coords, index_col=None)
        rows, columns = fov_table.shape
        assert rows == 1 and columns == 8 and fov_table.columns[0] == 'lower_left_x'
        ...
        for index in range(number_images):
            with fits.open(image_list[index]) as image_hdul:
                ...
                image_hdul[0].header['SFOVLLX'] = (fov_table.values[0, 0], 'Slot FOV Lower Left X')
                image_hdul[0].header['SFOVLLY'] = (fov_table.values[0, 1], 'Slot FOV Lower Left Y')
                image_hdul[0].header['SFOVULX'] = (fov_table.values[0, 2], 'Slot FOV Upper Left X')
                image_hdul[0].header['SFOVULY'] = (fov_table.values[0, 3], 'Slot FOV Upper Left Y')
                image_hdul[0].header['SFOVLRX'] = (fov_table.values[0, 4], 'Slot FOV Lower Right X')
                image_hdul[0].header['SFOVLRY'] = (fov_table.values[0, 5], 'Slot FOV Lower Right Y')
                image_hdul[0].header['SFOVURX'] = (fov_table.values[0, 6], 'Slot FOV Upper Right X')
                image_hdul[0].header['SFOVURY'] = (fov_table.values[0, 7], 'Slot FOV Upper Right Y')
    ...
    def update_sun_radius(self, image_list: list, sun_radius_observed: np.float32, sun_radius: np.float32):
        '''
    ...
    def create_level2_1_summed_image(self, image_list: list, output_dir: str):
        ...
        for i in range(running_summed_length):
            with fits.open(image_list[i]) as image_hdul:
                ...
                if i == 0:
                    level2_0_header['DATE_OBS'] = (image_hdul[0].header['DATE_OBS'], 'Date Observation')
                    level2_0_header['T_OBS'] = (image_hdul[0].header['T_OBS'], 'Telescope Observation')
                    level2_0_header['TELESCOP'] = (image_hdul[0].header['TELESCOP'], 'Telescope')
                    level2_0_header['INSTRUME'] = (image_hdul[0].header['INSTRUME'], 'Instrument')
                    level2_0_header['DEBIASED'] = (image_hdul[0].header['DEBIASED'], 'Bias Removed')
                    level2_0_header['DEDARKED'] = (image_hdul[0].header['DEDARKED'], 'Dark Removed')
                    level2_0_header['GAINADJ'] = (image_hdul[0].header['GAINADJ'], 'Gain Adjusted')
                    level2_0_header['BDPIXRPL'] = (image_hdul[0].header['BDPIXRPL'], 'Bad Pixels Replaced')
                    level2_0_header['DESPIKED'] = (image_hdul[0].header['DESPIKED'], 'Despiked')
                    level2_0_header['ABSORADJ'] = (image_hdul[0].header['ABSORADJ'], 'Absorption Adjusted')
                ...
                spike_replaced_values = spike_replaced_values.append(image_spike_replaced_values, ignore_index=True)
                if i == (running_summed_length - 1):
                    # Normalize data.
                    ...
                    if keywords_exist == True:
                        level2_0_header['SFOVLLX'] = (lower_left_x, 'SLOT FOV Lower Left X')
                        level2_0_header['SFOVLLY'] = (lower_left_y, 'SLOT FOV Lower Left Y')
                        level2_0_header['SFOVULX'] = (upper_left_x, 'SLOT FOV Upper Left X')
                        level2_0_header['SFOVULY'] = (upper_left_y, 'SLOT FOV Upper Left Y')
                        level2_0_header['SFOVLRX'] = (lower_right_x, 'SLOT FOV Lower Right X')
                        level2_0_header['SFOVLRY'] = (lower_right_y, 'SLOT FOV Lower Right Y')
                        level2_0_header['SFOVURX'] = (upper_right_x, 'SLOT FOV Upper Right X')
                        level2_0_header['SFOVURY'] = (upper_right_y, 'SLOT FOV Upper Right Y')
                    ...
                    hdu = fits.PrimaryHDU(data = light_summed_image.astype(np.float32), header=level2_0_header)
                    spike_dict = {'y':np.int32, 'x':np.int32, 'value':np.float32}
                    spike_replaced_values = spike_replaced_values.astype(spike_dict)
                    despike_table = Table.from_pandas(spike_replaced_values)
                    depike_table_hdu = fits.table_to_hdu(despike_table)
                    hdu_list = fits.HDUList([hdu, image_hdul[1].copy(), depike_table_hdu, image_hdul[3].copy()])
        ...
        image_output_file = output_dir + "magixs_L1.5_" + image_timestamp_begin + "_" + image_timestamp_end + "_summed_image.fits"
        hdu_list.writeto(image_output_file, overwrite=True)

    def create_level2_3_running_summed_images(self, image_list: list, running_summed_length: np.int32, output_dir: str):
        '''
    ...
    def perform_level2_0_elasticnet_inversion(self, image_list: list, rsp_func_cube_file: str, rsp_dep_name: str, rsp_dep_list: tp.Union[list, None], solution_fov_width: np.int32, smooth_over: str, field_angle_range: tp.Union[list, None], image_mask_file: tp.Union[str, None], level: str, detector_row_range: tp.Union[list, None], output_dir: str):
        ...
        if num_images > 0:
            for index in range(len(image_list)):
                ...
                inversion = Inversion(rsp_func_cube_file=rsp_func_cube_file,
                                      rsp_dep_name=rsp_dep_name, rsp_dep_list=rsp_dep_list,
                                      solution_fov_width=solution_fov_width, smooth_over=smooth_over,
                ...
                # alpha = 1e-3
                # rho = 0.8
                model = enet(alpha=alpha, l1_ratio=rho, max_iter=50000, precompute=True, positive=True, fit_intercept=True, selection='cyclic')
                #model = enet(alpha=alpha, l1_ratio=rho, max_iter=100000, precompute=True, positive=True, fit_intercept=True, selection='random')
                ...
                start = time.time()
                inversion.invert(inv_model, output_dir, output_file_prefix=basename, output_file_postfix="", level=level, detector_row_range=detector_row_range)
                end = time.time()
                print("Inversion Time =", end - start)

    def perform_level2_0_lassolars_inversion(self, image_list: list, rsp_func_cube_file: str, rsp_dep_name: str, rsp_dep_list: tp.Union[list, None], solution_fov_width: np.int32, smooth_over: str, field_angle_range: tp.Union[list, None], image_mask_file: tp.Union[str, None], level: str, detector_row_range: tp.Union[list, None], output_dir: str):
        '''
        ...
        if num_images > 0:
            for index in range(len(image_list)):
                ...
                #alpha = 1e-5 * 3
                alpha = 6e-3
                model = llars(alpha=alpha, max_iter=50000, normalize=False, precompute=True, positive=True, fit_intercept=True)
    ...
    def create_level2_0_spectrally_pure_images(self, image_list: list, gnt_file: str, rsp_dep_list: list, output_dir: str):
        '''
                ...
                for index in range(len(image_list)):
                    ...
                    with fits.open(image_list[index]) as em_hdul:
                        ...
                        for gnt_num in range(num_gnts):
                            gnt_image = (em_data_cube[:,:,0:num_rsp_deps] * 10**26 * gnt_values[gnt_num,0:num_rsp_deps]).sum(axis=2)
                            #gnt_image = (em_data_cube[:,:,0:num_rsp_deps] * gnt_values[gnt_num,0:num_rsp_deps]).sum(axis=2)
                        ...
                        em_hdul[0].header['IWTBLFMT'] = (ion_wavelength_table_format, 'Ion/Wavelength Table Format')
                        # Add binary table.
                        gnt_index_list = list(range(len(ion_wavelength_values)))
                        col1 = fits.Column(name='index', format='1I', array=gnt_index_list)
                        col2 = fits.Column(name=ion_wavelength_name, format='15A', array=ion_wavelength_values)
    ...
    def update_level2_pointing(self, image_list: list, level1_wcs_table: str, solar_fov_coords: str):
        number_images = len(image_list)
        ...
        for index in range(number_images):
            with fits.open(image_list[index]) as image_hdul:
                ...
                try:
                    fa_min = float(image_hdul[0].header['FA_MIN'])
                    fa_dlt = float(image_hdul[0].header['FA_DLT'])
                    fa_num = float(image_hdul[0].header['FA_NUM'])
                    ...
                    #level_2_x_wcs_table.iloc[[0], [3]] = (fa_min + (((fa_dlt) * (fa_num - 1)) / 2)) + level_2_x_wcs_table.values[0, 3]
                    ...
                    level_2_x_wcs_table.iloc[[0], [3]] = level_2_x_wcs_table.values[0, 3] - math.cos(90 - roll) * (2.8 * (level_1_y_pixel - level_2_x_wcs_table.values[0, 5]))
                    #level_2_x_wcs_table.iloc[[0], [6]] = level_2_x_wcs_table.values[0, 6] + (2.8 * (level_2_x_wcs_table.values[0, 5] - level_1_y_pixel))
                    level_2_x_wcs_table.iloc[[0], [6]] = level_2_x_wcs_table.values[0, 6] - math.sin(90 - roll) * (2.8 * (level_1_y_pixel - level_2_x_wcs_table.values[0, 5]))
                except KeyError:
                ...
                try:
                    ...
                    data_product_level = ''
                assert data_product_level == '2.1' or data_product_level == '2.3' or data_product_level == '2.4'
                image_hdul[0].header['SFOVLLX'] = (fov_table.values[0, 0], 'Slot FOV Lower Left X')
                image_hdul[0].header['SFOVLLY'] = (fov_table.values[0, 1], 'Slot FOV Lower Left Y')
                image_hdul[0].header['SFOVULX'] = (fov_table.values[0, 2], 'Slot FOV Upper Left X')
                image_hdul[0].header['SFOVULY'] = (fov_table.values[0, 3], 'Slot FOV Upper Left Y')
                ...
                try:
                    level_2_x_wcs_table_hdu = image_hdul[2]
    ...
    def create_level3_0_color_color_plot(self, spectrally_pure_data_cube: str, wavelength_list: list, saturation: float, lambda_scale: float, output_plot_filename: str):
        '''
        ...
        with fits.open(spectrally_pure_data_cube) as spdc_hdul:
            ...
            dep_data = np.zeros((len(wavelength_list), image_height, num_slits), dtype=np.float32)
            for i in range(len(wavelength_list)):
                for j in range(len(wavelengths)):
                    if wavelength_list[i] == wavelengths[j]:
                        ...
                        print(wavelength_list[i], np.max(dep_data[i]), np.min(dep_data[i]))
                        break
            ...
            for dep_index in range(len(wavelength_list)):
                ...
                dep_data[dep_index] = np.maximum(np.minimum(dep_data[dep_index], saturation), 0.0)

            max_value = np.amax(dep_data)

            if (len(wavelength_list) == 4):
                slit_data = dep_data[3]
                slit_max_value = np.max(slit_data)
    ...
    def create_summed_noisy_images(self, image_list: list, num_noisy_images: np.int32, output_dir_path: str, output_file_post_fix: str = "_Summed_Image"):
        ...
        summed_image_filename = output_dir_path + "Original" + output_file_post_fix + ".fits"
        level1_5_header = fits.Header()
        level1_5_header['IMG_EXP'] = (summed_image_exposure_time, 'Image Exposure')
        level1_5_header['MEAS_EXP'] = (summed_image_exposure_time, 'Measurement Exposure')
        hdu = fits.PrimaryHDU(data = summed_image, header = level1_5_header)
        hdu.writeto(summed_image_filename, overwrite=True)

        self.create_noisy_images(summed_image_filename, num_noisy_images, output_dir_path, output_file_post_fix)

    def create_noisy_images(self, image_file: str, num_noisy_images: np.int32, output_dir_path: str, output_file_post_fix: str):
    ...
    def create_image_list(self, input_dir: str, image_sequence_number_list: list) -> list:
        image_list = []
        os.chdir(input_dir)
        for file in glob.glob("*.fits"):
            with fits.open(file) as image_hdul:

...
class Image:
    ...
    def __post_init__(self):
        ...
        self.angle = self.camera_angle * np.pi / 180.0
        self.calculate_detector_dispersion(
    ...
    def calculate_detector_dispersion(
        self,
        element_temperature,
        element_mass,
        element_rest_wavelength,
        sigma_psf,
        pixel_delta_wavelength,
    ):
        thermal_velocity = (
            np.sqrt(k_B.value * element_temperature / element_mass) / 1.0e3
        )
        self.amplitude = 1.0 / (thermal_velocity * sqrt(2 * np.pi))
        self.width_of_pix_in_km_s = (
            pixel_delta_wavelength / element_rest_wavelength * (c.value / 1.0e3)
        )
    ...
    def crop_roi(self, lower, upper):
        ...
        crop_roi_coords.append(
            [int(np.rint(pixel_y.value)), int(np.rint(pixel_x.value))]
        )

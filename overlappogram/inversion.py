...
class Inversion:
    ...
    def invert(
        ...
        em_data_cube = np.zeros(
            (self.image_height, self.num_slits, self.num_response_files),
            dtype=np.float32,
        )
        ...
        for image_row_number in range(self.image_height):
            ...
            for slit_num in range(self.num_slits):
                if self.smooth_over == "dependence":
                    slit_em = em[
                        slit_num
                        * self.num_response_files : (slit_num + 1)
                        * self.num_response_files
                    ]
                else:
        ...
        col2 = fits.Column(
            name=self.rsp_dep_name,
            format=self.rsp_dep_desc_fmt,
            array=self.rsp_dep_list,
        )
